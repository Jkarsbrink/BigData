    ...
    def step1(raw_data):
        ...
        for i, line in enumerate(data):  # Moves question time to one row before. So can remove prior from that.
            ...
            if i < number_rows and i > 0:
                ...
                line[6] = str(
                    int(round((int(float(extra_data[i + 1][6])) / 1000), 0)))  # Converts time from ms to seconds
            ...
            if line[
                5] == '-1':  # answered_correctly: (int8) if the user responded correctly. Read -1 as null, for lectures.
